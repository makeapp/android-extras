...
public class PustDemoActivity extends Activity {
    ...
    public static final String[] type = new String[] { "获取Token(tmid)", "启动service或者发心跳", "标签&LBS测试"};
        ...
        gridView.setOnItemClickListener(new OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
                ...
                if (type[0].equals(clickText)) {
                    ...
                    // 启动Push服务或发心跳
                    String action = "com.huawei.android.push.intent.REFRESH_PUSH_CHANNEL";
                    Intent newIntent = new Intent(action);
                    newIntent.setFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
                    Log.i(TAG, "sendBroadcast the acrion is :" + action);
                    sendBroadcast(newIntent);

                } else if (type[2].equals(clickText)) {
                    // 打开标签、LBS上报页面
